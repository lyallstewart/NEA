...
class RequestWrapper {
  ...
  processBody() {
    ...
    return new Promise<boolean>((resolve, reject) => {
      ...
      if (["POST", "PUT", "DELETE"].includes(this.req.method)) {
        ...
        resolve(false); // Body not expected for GET etc
      }
    });
  }
  ...
  setCookies() {
    ...
    const cookies = [];
    this.outgoingCookies.forEach((c: { [key: string]: string | number }) => {
  ...
  sendSuccessResponse(body, code = 200) {
    this.setCookies();
    this.res.writeHead(code);
